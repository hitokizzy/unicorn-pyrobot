...
from unicorns import BRAIN, HELP, LOGS, TEMP_SETTINGS
from unicorns.modules.ban import get_reason
...
def globals_init():
    try:
        ...
        sql = import_module('geezram.sql.gban_sql')
        sql2 = import_module('geezram.sql.gmute_sql')
...
@unicorn(pattern='^.gban', compat=False)
...
@unicorn(pattern='^.(ung|gun)ban', compat=False)
...
@unicorn(pattern='^.gmute', compat=False)
...
@unicorn(pattern='^.(ung|gun)mute', compat=False)
