...
from unicorns import HELP, LOG_ID, LOGS
...
def filters_init():
    try:
        ...
        sql = import_module('geezram.sql.filters_sql')
...
def filter_incoming(message):
    ...
    try:
        from geezram.sql.filters_sql import get_filters
...
@unicorn(pattern='^.addfilter')
def add_filter(message):
    try:
        from geezram.sql.filters_sql import add_filter
    except BaseException:
...
@unicorn(pattern='^.stop')
def stop_filter(message):
    try:
        from geezram.sql.filters_sql import remove_filter
    except BaseException:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
@unicorn(pattern='^.stopall$')
def stop_filter_all(message):
    try:
        from geezram.sql.filters_sql import get_filters, remove_filter
    except BaseException:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
    filters = get_filters(message.chat.id)
