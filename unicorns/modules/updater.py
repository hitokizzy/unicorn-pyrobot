...
from unicorns import HELP, HEROKU_APPNAME, HEROKU_KEY, REPO_URL
...
def upstream(ups):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head('unicorn', origin.refs.unicorn)
        repo.heads.unicorn.set_tracking_branch(origin.refs.unicorn)
        repo.heads.unicorn.checkout(True)
    ...
    if force_update:
        ...
        edit(ups, f'`{get_translation("updateunicornBot")}`')
