...
from unicorns import HELP, UNI_LANG
...
def do_gsearch(query, page):
    ...
    page = find_page(page)
    temp = f'/search?q={query}&start={find_page(page)}&hl={UNI_LANG}'
...
@unicorn(pattern='^.d(uckduck|d)go')
def ddgo(message):
    query = extract_args(message)
    if len(query) < 1:
        edit(message, f'`{get_translation("wrongCommand")}`')
...
def wiki(message):
    ...
    edit(message, get_translation('unicornQuery', ['**', '`', args, result]))
...
def imeichecker(message):
    ...
    try:
        ...
        reply_text = (
            f"<b>Query Date</b> ⇾ <code>{result['queryDate']}</code>\n"
            ...
            f"<b>Powered by</b> ⇾ <a href='https://github.com/TeamDerUntergang/Telegram-unicornUserBot'>unicorn ♥</a>\n"
        )
