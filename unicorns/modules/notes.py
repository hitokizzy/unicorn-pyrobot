...
def notes_init():
    try:
        ...
        sql = import_module('geezram.sql.notes_sql')
...
@unicorn(pattern='^.notes$')
def notes(message):
    try:
        from geezram.sql.notes_sql import get_notes
...
@unicorn(pattern=r'^.save')
def save_note(message):
    try:
        from geezram.sql.notes_sql import add_note
    except AttributeError:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
@unicorn(pattern=r'^.clear')
def clear_note(message):
    try:
        from geezram.sql.notes_sql import rm_note
    except AttributeError:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
def get_note(message):
    try:
        try:
            from geezram.sql.notes_sql import get_note
