...
from unicorns import HELP, HEROKU_APPNAME, HEROKU_KEY
from geezram.core import edit, get_translation, reply_doc, unicorn, send_log


@unicorn(pattern='^.usage$')
def dyno(message):
    if not HEROKU_KEY:
...
@unicorn(pattern='^.(restart|yb)$')
def _restart(message):
    return restart(message)


@unicorn(pattern='^.d(restart|yb)$')
...
def shutdown(message):
    ...
    heroku_app.scale_formation_process('unicorn', 0)


@unicorn(pattern='^.logs$')
def dyno_logs(message):
    ...
    filename = 'unicorn_heroku_log.txt'
