...
from unicorns import HELP, LOGS, PM_AUTO_BAN, PM_MSG_COUNT, PM_UNAPPROVED, TEMP_SETTINGS
from unicorns.modules.chat import is_muted
from geezram.core import edit, get_translation, reply, unicorn, send_log
...
def pmpermit_init():
    try:
        ...
        sql = import_module('geezram.sql.pm_permit_sql')
...
def permitpm(client, message):
    if not PM_AUTO_BAN:
        ...
        if message.chat.id != 777000:
            try:
                from geezram.sql.pm_permit_sql import is_approved
...
def auto_accept(client, message):
    ...
    if message.chat.id not in [self_user.id, 777000]:
        try:
            from geezram.sql.pm_permit_sql import approve, is_approved
        except BaseException:
            return False
...
@unicorn(outgoing=True, pattern='^.notifoff$')
...
@unicorn(outgoing=True, pattern='^.notifon$')
def notifon(message):
    try:
...
@unicorn(outgoing=True, pattern='^.approve$', compat=False)
def approvepm(client, message):
    try:
        from geezram.sql.pm_permit_sql import approve
    except BaseException:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
@unicorn(outgoing=True, pattern="^.disapprove$")
def disapprovepm(message):
    try:
        from geezram.sql.pm_permit_sql import dissprove
    except BaseException:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
