...
def snips_init():
    try:
        ...
        sql = import_module('geezram.sql.snips_sql')
...
@unicorn(pattern='^.addsnip')
def save_snip(message):
    try:
        from geezram.sql.snips_sql import add_snip
...
@unicorn(pattern='^.snips$')
def snip_list(message):
    try:
        from geezram.sql.snips_sql import get_snips
...
@unicorn(pattern='^.remsnip')
def delete_snip(message):
    try:
        from geezram.sql.snips_sql import remove_snip
    except AttributeError:
        edit(message, f'`{get_translation("nonSqlMode")}`')
        return
...
def get_snip(message):
    try:
        try:
            from geezram.sql.snips_sql import get_snip
        except BaseException:
            edit(message, f'`{get_translation("nonSqlMode")}`')
            return
