...
from unicorns import HELP, SUPPORT_GROUP
...
@unicorn(pattern='^.chatid$', private=False)
def chatid(message):
    edit(message, get_translation('chatidResult', ['`', str(message.chat.id)]))


@unicorn(pattern='^.invitelink$', compat=False, admin=True, private=False)
...
@unicorn(pattern='^.kickme$', compat=False, private=False)
def kickme(client, message):
    edit(message, f'`{get_translation("kickmeResult")}`')
    client.leave_chat(message.chat.id, 'me')


@unicorn(pattern='^.support$')
def support(message):
    edit(message, get_translation('supportResult', [SUPPORT_GROUP]), preview=False)


@unicorn(pattern='^.founder')
def founder(message):
    edit(message, get_translation('founderResult', ['`', '**']), preview=False)


@unicorn(pattern='^.readme$')
def readme(message):
    edit(
        message,
        '[unicorn README.md](https://github.com/hitokizzy/'
        'unicorn/blob/unicorn/README.md)',
        preview=False,
    )


@unicorn(pattern='^.repo$')
def repo(message):
    edit(
        message,
        '[unicorn Repo](https://github.com/hitokizzy/' 'unicorn)',
        preview=False,
    )


@unicorn(pattern='^.repeat')
def repeat(message):
...
@unicorn(pattern='^.report$', compat=False, private=False)
