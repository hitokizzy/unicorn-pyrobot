...
import geezram.translator as _tr
...
UNI_LANG = environ.get('UNI_LANG', 'en')


def get_translation(transKey, params: list = None):
    ret = _tr.get_translation(UNI_LANG, transKey)
...
# unicornBot Session
SESSION = environ.get('SESSION', 'unicorn')

# unicornBot repo url for updater
REPO_URL = environ.get(
    'REPO_URL', 'https://github.com/hitokizzy/unicorn'
)
...
class PyroClient(Client):
    ...
    def start(self):
        ...
        LOGS.info(get_translation('unicornVersion', [BOT_VERSION]))
...
app = PyroClient('unicorn', api_id=API_ID, api_hash=API_HASH, session_string=SESSION)
...
def __import_modules():
    ...
    for module in modules:
        try:
            import_module(f'unicorns.modules.{module}')
        except Exception:
            if LOG_VERBOSE:
